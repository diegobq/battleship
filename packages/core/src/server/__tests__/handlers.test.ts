...
import { HandlerContext, HandlerDeps, handleClientMessage } from '../ws/handlers';
...
let shipSeq = 0;
function nextShipId() { return `s${shipSeq++}`; }

function buildPlacementGame(): { game: GameState; hostShipId: string; joinerShipId: string } {
  shipSeq = 0;
  ...
  const game = addSecondPlayer(base, createPlayer('joiner', 'Joiner'), { idFactory: nextShipId });
...
function buildPlayingGame() {
  ...
  let g = placeFleet(placement, 'host', [{ shipId: hostShipId, r: 0, c: 0, orientation: 'horizontal' }], { clock, rng });
  g = placeFleet(g, 'joiner', [{ shipId: joinerShipId, r: 7, c: 0, orientation: 'horizontal' }], { clock, rng });
  return g;
}
...
describe('handleClientMessage', () => {
  ...
  describe('PING', () => {
    it('sends PONG only to the sender', () => {
      const { deps, hostSock, joinerSock } = makeSetup(buildPlacementGame().game);
  ...
  describe('PLACE_FLEET', () => {
    it('broadcasts state after first placement; game stays in placement', () => {
      ...
      handleClientMessage(deps, { gameId: 'g1', playerId: 'host' }, {
        type: 'PLACE_FLEET',
        payload: { placements: [{ shipId: hostShipId, r: 0, c: 0, orientation: 'horizontal' }] },
      });
    ...
    it('transitions to playing and starts the timer once both players place', () => {
      ...
      handleClientMessage(deps, { gameId: 'g1', playerId: 'joiner' }, {
        type: 'PLACE_FLEET',
        payload: { placements: [{ shipId: joinerShipId, r: 7, c: 0, orientation: 'horizontal' }] },
      });
  ...
  describe('SHOOT', () => {
    it('cancels the timer, broadcasts SHOT_RESULT + state, starts a new timer', () => {
      ...
      handleClientMessage(deps, { gameId: 'g1', playerId: shooterId }, {
        type: 'SHOOT',
        payload: { r: 5, c: 5 },
      });
    ...
    it('does not restart the timer when the shot ends the game', () => {
      ...
      const shooterId = playing.activePlayerId!;
      const targetId = Object.keys(playing.players).find((id) => id !== shooterId)!;
    ...
    it('sends WRONG_TURN error when it is not the player\'s turn', () => {
      ...
      const wrongId = Object.keys(playing.players).find((id) => id !== playing.activePlayerId)!;
      const wrongSock = wrongId === 'host' ? hostSock : joinerSock;

      handleClientMessage(deps, { gameId: 'g1', playerId: wrongId }, {
        type: 'SHOOT',
        payload: { r: 0, c: 0 },
      });
    ...
    it('sends ALREADY_SHOT error when firing at a previously-hit cell', () => {
      ...
      handleClientMessage(deps, { gameId: 'g1', playerId: shooterId }, {
        type: 'SHOOT', payload: { r: targetPos.r, c: targetPos.c },
      });
      ...
      handleClientMessage(deps, { gameId: 'g1', playerId: shooterId }, {
        type: 'SHOOT', payload: { r: 9, c: 9 }, // out of bounds
      });
  ...
  describe('LEAVE_GAME', () => {
    it('broadcasts finished state with opponent as winner', () => {
      ...
      handleClientMessage(deps, { gameId: 'g1', playerId: leaverId }, { type: 'LEAVE_GAME' });
