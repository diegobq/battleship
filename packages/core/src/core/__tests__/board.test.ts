...
describe('expandShipCells', () => {
  ...
  it('expands a length-1 ship to a single cell', () => {
    expect(expandShipCells({ r: 5, c: 5 }, 1, 'horizontal')).toEqual([{ r: 5, c: 5 }]);
  });
});
...
describe('canPlace', () => {
  ...
  it('rejects placement that collides with an existing ship', () => {
    const grid = createEmptyGrid();
    const result = applyPlacement(grid, ship('s1', 3), { r: 0, c: 0 }, 'horizontal');
...
describe('applyPlacement', () => {
  ...
  it('returns a placed ship with positions and orientation set', () => {
    const grid = createEmptyGrid();
    const result = applyPlacement(grid, ship('s1', 2), { r: 3, c: 4 }, 'vertical');
    expect(result).not.toBeNull();
  ...
  it('returns null when placement is invalid', () => {
    const grid = createEmptyGrid();
    expect(applyPlacement(grid, ship('s1', 3), { r: 0, c: 6 }, 'horizontal')).toBeNull();
  });
...
describe('applyShot', () => {
  ...
  it('marks a ship cell as hit', () => {
    const placed = applyPlacement(createEmptyGrid(), ship('s1', 2), { r: 0, c: 0 }, 'horizontal');
...
describe('countHiddenCells', () => {
  ...
  it('excludes hit and miss cells from the count', () => {
    const placed = applyPlacement(createEmptyGrid(), ship('s1', 1), { r: 0, c: 0 }, 'horizontal');
