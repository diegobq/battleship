...
## 2. Graceful User Disconnection During Game
...
*   **Design:** 
    ...
    4. **Session Resumption:** If the user reopens the app on *any* device, the initial API call fetches their active sessions. They are automatically routed back to the game. The WebSocket authenticates, fetches the current board state, and the match resumes seamlessly, removing the blocking modal for the opponent.
