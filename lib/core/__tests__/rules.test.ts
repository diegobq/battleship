...
describe('validateShot', () => {
  ...
  it('throws OUT_OF_BOUNDS for invalid coordinates', () => {
    expect(() => validateShot(makeGame(), 'p1', -1, 0)).toThrow(/out of bounds/i);
    expect(() => validateShot(makeGame(), 'p1', 0, 99)).toThrow(/out of bounds/i);
  });
...
describe('isGameOver', () => {
  function sunkShip(): Ship {
    return { id: 's', type: 'Submarine', length: 1, hits: 1, positions: [], placed: true };
  }
  function aliveShip(): Ship {
    return { id: 's', type: 'Submarine', length: 1, hits: 0, positions: [], placed: true };
  }
