...
function useGameContext(gameId: string, playerId: string) {
  ...
  const [turnExpiredPlayerId, setTurnExpiredPlayerId] = useState<string | null>(
    null,
  );
  ...
  const shoot = useCallback(
    (r: number, c: number) =>
      send(JSON.stringify({ type: "SHOOT", payload: { r, c } })),
    [send],
  );
...
export function GameProvider({
  ...
  const {
    ...
    leaveGame,
    dismissError,
  } = useGameContext(gameId, playerId);
