...
class MockWebSocket {
  ...
  receive(d: string) { this.onmessage?.({ data: d }); }
}
...
function wrapper({ children }: { children: React.ReactNode }) {
  return <GameProvider gameId="g1" playerId="p1">{children}</GameProvider>;
}

function socket(): MockWebSocket { return MockWebSocket.last; }
...
describe('GameProvider / useGame', () => {
  ...
  it('updates state on GAME_STATE_UPDATE message', () => {
    ...
    act(() => {
      socket().receive(JSON.stringify({ type: 'GAME_STATE_UPDATE', payload: { state: fakeState } }));
    });
    expect(result.current.state).toMatchObject({ id: 'g1', status: 'placement' });
  });
  ...
  it('captures SHOT_RESULT with a timestamp', () => {
    ...
    act(() => {
      socket().receive(JSON.stringify({
        type: 'SHOT_RESULT',
        payload: { shooterId: 'p1', r: 0, c: 0, hit: true, scoreAwarded: 10, cellStatus: 'hit' },
      }));
    });
  ...
  it('sets turnExpiredPlayerId on TURN_TIMEOUT and clears it after 2 s', () => {
    ...
    act(() => { socket().open(); });
    act(() => {
      socket().receive(JSON.stringify({ type: 'TURN_TIMEOUT', payload: { playerId: 'p1' } }));
    });
  ...
  it('sets errorMessage on ERROR message and clears it via dismissError', () => {
    ...
    act(() => { socket().open(); });
    act(() => {
      socket().receive(JSON.stringify({ type: 'ERROR', payload: { message: 'bad' } }));
    });
  ...
  it('placeFleet() sends a PLACE_FLEET message when socket is open', () => {
    ...
    act(() => { socket().open(); });
    act(() => {
      result.current.placeFleet([{ shipId: 's1', r: 0, c: 0, orientation: 'horizontal' }]);
    });
