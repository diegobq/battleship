...
describe("parseClientMessage", () => {
  ...
  it("parses a valid SHOOT", () => {
    expect(
      parseClientMessage(
        JSON.stringify({ type: "SHOOT", payload: { r: 3, c: 4 } }),
      ),
    ).toEqual({
      type: "SHOOT",
      payload: { r: 3, c: 4 },
  ...
  it("rejects SHOOT with non-integer coordinates", () => {
    expect(() =>
      parseClientMessage(
        JSON.stringify({ type: "SHOOT", payload: { r: 1.5, c: 2 } }),
      ),
    ).toThrow(/integer/);
  });
    ...
    expect(() =>
      parseClientMessage(
        JSON.stringify({
          type: "PLACE_FLEET",
          payload: {
            placements: [{ shipId: "s1", r: 0, c: 0, orientation: "diagonal" }],
          },
        }),
      ),
...
describe("serializeServerMessage", () => {
  it("serializes a PONG with no payload", () => {
    expect(serializeServerMessage({ type: "PONG" })).toBe(
