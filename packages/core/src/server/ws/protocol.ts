...
function parsePlaceFleet(payload: unknown): ClientMessage {
  ...
  return { type: 'PLACE_FLEET', payload: { placements: payload.placements as ShipPlacement[] } };
}

function parseShoot(payload: unknown): ClientMessage {
  if (!isRecord(payload) || typeof payload.r !== 'number' || typeof payload.c !== 'number') {
...
export function sanitizeGameStateFor(state: GameState, viewerId: string): GameState {
  const players = { ...state.players };
  for (const [pid, p] of Object.entries(state.players)) {
    if (pid === viewerId) continue;
    players[pid] = {
      ...p,
      grid: p.grid.map((row) => row.map((cell) => (cell === 'ship' ? 'empty' : cell))),
      ships: p.ships.map((s) => ({
        ...s,
