...
function placeAllShipsAt(player: PlayerState, startRow: number): ShipPlacement[] {
...
describe('placeFleet', () => {
  ...
  it('transitions to playing once both players have placed', () => {
    ...
    let g = placeFleet(game, hostId, placeAllShipsAt(game.players[hostId], 0), { clock, rng });
    g = placeFleet(g, joinerId, placeAllShipsAt(game.players[joinerId], 0), { clock, rng });
    expect(g.status).toBe('playing');
    expect(g.activePlayerId === hostId || g.activePlayerId === joinerId).toBe(true);
  ...
  it('rejects when ship count mismatches', () => {
    const { game, hostId } = newGameInPlacement();
    expect(() =>
      placeFleet(game, hostId, [], { clock: makeFakeClock(), rng: makeSeededRng(1) }),
    ).toThrow(/all 3 ships/);
  });
  ...
  it('rejects when a ship is placed off the board', () => {
    ...
    expect(() =>
      placeFleet(game, hostId, bad, { clock: makeFakeClock(), rng: makeSeededRng(1) }),
  ...
  it('rejects when ships collide', () => {
    ...
    expect(() =>
      placeFleet(game, hostId, colliding, { clock: makeFakeClock(), rng: makeSeededRng(1) }),
    ).toThrow();
  });
});
...
function startPlayingGame(): {
  ...
  g = placeFleet(g, setup.joinerId, placeAllShipsAt(g.players[setup.joinerId], 0), { clock, rng });
...
describe('processShot — hit / miss / sunk / game over', () => {
  it('marks a miss, resets streak, alternates turn', () => {
    ...
    const out = processShot(game, shooterId, 7, 7, { clock: makeFakeClock(500) });
  ...
  it('marks a hit, increments streak, alternates turn', () => {
    ...
    const { game, shooterId, targetId } = startPlayingGame();
    const out = processShot(game, shooterId, 2, 0, { clock: makeFakeClock(500) });
  ...
  it('transitions to finished and sets winnerId when all opponent ships are sunk', () => {
    ...
    const fire = (r: number, c: number) => {
      const out = processShot(game, shooterId, r, c, { clock: makeFakeClock(0) });
  ...
  it('floors player score at zero in Risk mode after consecutive misses', () => {
    ...
    g = placeFleet(g, setup.joinerId, placeAllShipsAt(g.players[setup.joinerId], 0), { clock, rng });
    g = { ...g, activePlayerId: setup.hostId };
    const out = processShot(g, setup.hostId, 7, 7, { clock: makeFakeClock(500) });
  ...
  it('throws GameRuleError for an out-of-turn shot', () => {
    ...
    expect(() => processShot(game, targetId, 0, 0, { clock: makeFakeClock(0) })).toThrow(
      GameRuleError,
    );
  });
  ...
  it('uses injected clock to compute elapsed time for reflex bonus (Elite)', () => {
    ...
    const rng = makeSeededRng(7);
    let g = placeFleet(setup.game, setup.hostId, placeAllShipsAt(setup.game.players[setup.hostId], 0), {
      clock: makeFakeClock(0),
      rng,
    });
    g = placeFleet(g, setup.joinerId, placeAllShipsAt(g.players[setup.joinerId], 0), {
      clock: makeFakeClock(0),
      rng,
    });
    g = { ...g, activePlayerId: setup.hostId, lastActionTime: 0 };
    // Shoot a hit (target row 2 col 0 = submarine) within 1s -> reflex bonus applies.
    const out = processShot(g, setup.hostId, 2, 0, { clock: makeFakeClock(1_000) });
    expect(out.result.hit).toBe(true);
...
describe('handleTurnTimeout', () => {
  it('flips the active player and resets the timed-out streak', () => {
    const { game, shooterId, targetId } = startPlayingGame();
    const expired = { ...game, players: { ...game.players, [shooterId]: { ...game.players[shooterId], consecutiveHits: 3 } } };
  ...
  it('throws when called on a non-playing game', () => {
    ...
    expect(() => handleTurnTimeout(lobby, { clock: makeFakeClock(0) })).toThrow(GameRuleError);
  });
});
