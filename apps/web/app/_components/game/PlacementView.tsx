...
function usePlacement(initialShips: Ship[]): Placement {
  ...
  const previewCells = useMemo<Coordinate[]>(() => {
    ...
    const ship = reducerState.ships.find(
      (s) => s.id === reducerState.selectedShipId,
    );
    if (!ship || ship.placed) return [];
    return expandShipCells(hovered, ship.length, reducerState.orientation);
  }, [
    hovered,
    reducerState.selectedShipId,
    reducerState.orientation,
    reducerState.ships,
  ]);
  const previewInvalid =
    hovered !== null &&
    !canPreviewPlacement(reducerState, hovered.r, hovered.c);
  return {
    reducerState,
    dispatch,
    hovered,
...
export default function PlacementView() {
  ...
  if (initialShips.length === 0)
    return (
      <p className="p-4">Server hasn&apos;t sent your fleet yet. Hang tight…</p>
    );
  if (placement.submitted || me.ready)
    return (
      <p className="p-4 text-center">Fleet locked in. Waiting for opponent…</p>
    );
    ...
    <main className="flex-1 mx-auto w-full max-w-3xl px-4 py-6 flex flex-col gap-6">
      <header>
        ...
        <p className="opacity-70 text-sm">
          Pick a ship from the palette, choose orientation, and tap a cell to
          place. Ships cannot overlap or extend off the board.
        </p>
      </header>
...
function PlacementBoardSection({ placement }: { placement: Placement }) {
  return (
    <Board
      ...
      onCellHover={(r, c) =>
        placement.setHovered(r === null || c === null ? null : { r, c })
      }
...
function PlacementActionButtons({
  reducerState,
  dispatch,
  onReady,
  onLeave,
