...
describe("parseCreateGameRequest", () => {
  ...
  it("rejects missing playerName", () => {
    expect(() => parseCreateGameRequest({ mode: "Classic" })).toThrow(
      /playerName/,
    );
  });
  ...
  it("rejects unknown mode", () => {
    expect(() =>
      parseCreateGameRequest({ mode: "Hard", playerName: "Bob" }),
    ).toThrow(/mode/);
  });
...
describe("parseJoinGameRequest", () => {
  it("parses a valid payload", () => {
    expect(
      parseJoinGameRequest({ gameId: "abc12345", playerName: "Bob" }),
    ).toEqual({
      gameId: "abc12345",
      playerName: "Bob",
    });
