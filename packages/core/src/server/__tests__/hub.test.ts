...
describe('WebSocketHub', () => {
  ...
  describe('sendTo', () => {
    ...
    it('sends the serialised message and returns true', () => {
      ...
      expect(JSON.parse(sock.send.mock.calls[0][0])).toMatchObject({ type: 'PONG' });
    });
  ...
  describe('broadcast', () => {
    ...
    it('passes each recipient playerId to the factory', () => {
      ...
      hub.register('g1', 'p2', mockSocket());
      const seen: string[] = [];
      hub.broadcast('g1', (pid) => { seen.push(pid); return { type: 'PONG' }; });
    ...
    it('is a no-op for an unregistered game', () => {
      expect(() => hub.broadcast('ghost', () => ({ type: 'PONG' }))).not.toThrow();
    });
  });
